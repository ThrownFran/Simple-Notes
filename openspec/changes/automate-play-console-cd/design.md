...
**Signing config sourced from `keystore.properties` (local) / injected secrets (CI), not committed anywhere.**
Standard Android pattern: a gitignored `keystore.properties` file at the repo root (with a committed `keystore.properties.example` template) holds `storeFile`, `storePassword`, `keyAlias`, `keyPassword` for local builds. `app/build.gradle`'s `signingConfigs.release` reads that file if present. In CI, the workflow decodes a base64-encoded keystore secret to a file and writes the same `keystore.properties` shape from secrets before invoking Gradle — so the Gradle logic has exactly one source-of-truth format regardless of environment. Alternative considered: reading raw environment variables directly in `build.gradle` — rejected because it forces two different code paths (env-var branch vs. properties-file branch) for what's conceptually the same input, and `keystore.properties` is the pattern most Android devs already expect.
