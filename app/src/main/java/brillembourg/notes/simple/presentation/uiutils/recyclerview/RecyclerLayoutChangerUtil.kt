package brillembourg.notes.simple.presentation.uiutils.recyclerview
...
fun buildLayoutManager(
    context: Context,
    layoutType: LayoutType,
): RecyclerView.LayoutManager {
    return if (layoutType == LayoutType.Staggered) {
        buildStaggeredManager(context)
    } else {
        buildVerticalManager(
            context,
        )
    }
}
...
fun changeLayout(
    ...
    recyclerView.apply {
        val isLayoutChangeNeeded: Boolean =
            when (type) {
...
fun getDragDirs(layoutType: LayoutType) =
    when (layoutType) {
