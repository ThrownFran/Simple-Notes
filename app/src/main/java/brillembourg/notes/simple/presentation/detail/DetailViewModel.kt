...
class DetailViewModel
    ...
    constructor(
        ...
        private fun getSavedUiStateFromDeath(): DetailUiState? = savedStateHandle.get<DetailUiState>(uiStateKey)

        private fun getSavedTaskFromDeath(): NotePresentationModel? = savedStateHandle.get<NotePresentationModel>(noteSavedStateKey)

        private fun getSavedNoteFromNav() = DetailFragmentArgs.fromSavedStateHandle(savedStateHandle).task
        ...
        //region Create
        private fun createTask(navigateBack: Boolean = true) {
                ...
                when (result) {
                    is Resource.Success -> {
                        ...
                        currentNotePresentation =
                            result.data.note.toPresentation(dateProvider).apply {
        ...
        private fun editTaskState(note: NotePresentationModel) {
            _uiDetailState.update {
                it.copy(
                    userInput =
                        _uiDetailState.value.userInput.copy(
                            title = note.title ?: "",
        ...
        private fun saveTask(navigateBack: Boolean = true) {
                    ...
                    when (result) {
                        is Resource.Success -> {
                            _uiDetailState.update { uiState ->
                                uiState.copy(
                                    selectCategories =
                                        _uiDetailState.value.selectCategories.copy(
                                            categories =
        ...
        fun onCategoryChecked(
            ...
            if (isChecked) {
                viewModelScope.launch {
                    val params =
                        AddCategoryToNoteUseCase.Params(
                ...
                viewModelScope.launch {
                    val params =
                        RemoveCategoryToNoteUseCase.Params(
                            currentNotePresentation!!.toDomain(dateProvider),
                            categoryUpdated.toDomain(),
                        )
