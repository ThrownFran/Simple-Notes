package brillembourg.notes.simple.domain.usecases.user
...
class GetFilterByCategoriesUseCaseTest {
    ...
    private fun mockGetFilterRepositorySuccess() {
        coEvery { repository.getFilter(any()) } returns
            flow {
    ...
    private fun mockGetCategoriesUseCaseError() {
        coEvery { getCategoriesUseCase.invoke(ofType(GetCategoriesUseCase.Params::class)) } returns
            flow {
                emit(Resource.Error(Exception("Error")))
            }
    }
    ...
    @Test
    fun `execute invokes getCategoriesUseCase`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `execute invokes getFilterIds from repository`() =
        runTest {
            // Arrange
            mockGetCategoriesUseCaseSuccess()
    ...
    @Test
    fun `execute success, returns filtered categories ids as Category model`() =
        runTest {
            // Arrange
            mockGetCategoriesUseCaseSuccess()
            mockGetFilterRepositorySuccess()
    ...
    @Test
    fun `execute get categories error, returns Error result`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `execute get filter error, returns Error result`() =
        runTest {
            // Arrange
            mockGetCategoriesUseCaseSuccess()
